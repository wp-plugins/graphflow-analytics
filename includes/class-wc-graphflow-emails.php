...
if ( ! class_exists( 'WC_Graphflow_Emails' ) ) {
	...
	class WC_Graphflow_Emails {
		...
		public function email_recommendations_shortcode( $atts ) {
			...
			$email_section = $this->generate_email_rec_section( $user_id, $title, $desc, $rows, $cols, $filters, $rec_type, $test, $coupon_args );
		...
		public function register_variable_replacements( $var, $email_data, $email, $queue_item ) {
			...
			$gf_shortcode = preg_replace( '|\]|', ' user_id=' . $user_id . ' is_fue=1]', $gf_shortcode );
			// use test data if the test flag is set
			if ( isset( $email_data['test'] ) && $email_data['test'] ) {
				$gf_shortcode = preg_replace( '|\]|', ' test=1 is_fue=1]', $gf_shortcode );
			}

			$GLOBALS['wc_graphflow']->get_api()->log->add("graphflow", print_r( array( $gf_var, $gf_shortcode ), true ) );
