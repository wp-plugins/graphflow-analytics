...
function get_cart_recommendations_email( $cart_products, $gf_user, $gf_num, $filters, $recType, $vars = array() ) {
	$gf_products = array();
	$gf_recommendations = $GLOBALS['wc_graphflow']->get_api()->get_product_recommendations( $cart_products, $gf_user, apply_filters( 'woocommerce_graphflow_cart_recommended_products_total', $gf_num ), $filters, $recType, true, $vars );
...
function get_user_recommendations( $gf_user, $gf_num, $filters, $recType, $vars = array() ) {
	$gf_products = array();
	$gf_recommendations = $GLOBALS['wc_graphflow']->get_api()->get_user_recommendations( $gf_user, apply_filters( 'woocommerce_graphflow_recommended_products_total', $gf_num ), $filters, $recType, $vars );
	if ( isset( $gf_recommendations->result ) ) {
		foreach ( $gf_recommendations->result as $item ) {
			$gf_products[] = $item->itemId;
		}
