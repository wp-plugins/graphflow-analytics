...
if ( ! class_exists( 'WC_Graphflow' ) ) {
	class WC_Graphflow {
		...
		public function __construct( $file ) {
			...
			// new WC 2.4 endpoint
			add_action( 'wc_ajax_graphflow_get_recs', array( $this, 'woocommerce_graphflow_recommendation_display_ajax' ) );
			...
			// Track order changes
			add_action( 'woocommerce_order_status_changed', array( $this, 'capture_order_status_changed' ), 10, 3 );
			...
			// Register hooks for displaying recommendations
		...
		public function capture_order_status_changed( $order_id, $old_status, $new_status ) {
			$this->get_api()->update_order_status( $order_id, $new_status );
		}
		 ...
		 * @param bool $force
		 */
		public function capture_historic_orders( $order_ids, $force = false ) {
			$order_data = array();
			$order_users = array();
			foreach ( $order_ids as $order_id ) {
				$exported = get_post_meta( $order_id, '_wc_graphflow_exported', true );
				if ( $force || 'yes' !== $exported ) {
					$order = $this->get_order( $order_id );
					if ( ! $order ) {
						$this->get_api()->log->add("graphflow", "Failed to get order for order_id: " . $order_id . "; request: " . $_SERVER['REQUEST_URI'] );
						continue;
		...
		public function capture_all_orders() {
			...
			$force = get_option( 'woocommerce_graphflow_force_order_export', 'no' ) == 'yes';
			...
			while ( !$finished ) {
				...
				$this->capture_historic_orders( $sales, $force );
				$current_page = $current_page + 1;
		...
		public function remove_storefront_upsells( ) {
		...
		public function display_recommendations_based_on_settings() {
			if ( 'yes' == get_option( 'woocommerce_graphflow_show_on_product' ) ) {
				add_action( 'after_setup_theme', array( $this, 'remove_storefront_upsells' ) );
